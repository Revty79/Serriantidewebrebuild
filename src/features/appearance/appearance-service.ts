...
import { assertPreliminaryAdministratorAccess, lockAdministratorRosterInTransaction } from "@/features/authorization/admin-roster-lock";
import { db } from "@/db";
...
export async function saveSiteAppearance(
  ...
  const now = new Date();
  await assertPreliminaryAdministratorAccess(administratorUserId);
  await db.transaction(async (tx) => {
    await lockAdministratorRosterInTransaction(tx);
    const [admin] = await tx.select({ userId: userRole.userId }).from(userRole)
      .where(and(eq(userRole.userId, administratorUserId), eq(userRole.role, "admin"))).limit(1);
