      ...
      <div className="mx-auto w-full max-w-7xl">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
                Site-wide visibility across active and archived Campaign content. Summaries appear as each system becomes available in the rebuild.
              </p>
            </div>
            ...
            {overviewCards.map((card) => {
              ...
              return (
                <article key={card.key} className="rounded-2xl border border-white/10 bg-black/35 p-5 shadow-xl backdrop-blur-md">
                  ...
                  <p className="mt-3 text-xl text-amber-200">{overview.availability[card.key] ? counts.total : "Not available yet"}</p>
                  <p className="mt-2 text-sm text-slate-400">
                    {overview.availability[card.key] && <><span className="text-emerald-200">{counts.active} active</span>
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-black/35 p-6 shadow-2xl backdrop-blur-md sm:p-7" aria-labelledby="shared-catalog-heading">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <p className="text-sm text-slate-400">Available libraries show their active and archived totals.</p>
          </div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            {overview.sharedCatalogs.map((catalog) => (
              ...
              >
                ...
                <p className="mt-3 text-2xl text-amber-200">{catalog.available ? catalog.total : "Not available yet"}</p>
                {catalog.available && <p className="mt-1 text-xs text-slate-400">{catalog.active} active · {catalog.archived} archived</p>}
              </article>
            ))}
          </div>
        </section>
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-2">
          <ContentSection title="Campaigns" counts={overview.counts.campaigns} available={overview.availability.campaigns} emptyMessage="No Campaigns found.">
          ...
          <ContentSection title="Player Characters" counts={overview.counts.playerCharacters} available={overview.availability.playerCharacters} emptyMessage="No Player Characters found.">
            {overview.playerCharacters.map((character) => (
              <CharacterRecord key={character.id} character={character} />
            ))}
          </ContentSection>

          <ContentSection title="Race NPCs" counts={overview.counts.raceNpcs} available={overview.availability.raceNpcs} emptyMessage="No Race NPCs found.">
            {overview.raceNpcs.map((character) => (
              <CharacterRecord key={character.id} character={character} />
            ))}
          </ContentSection>

          <ContentSection title="Creature NPCs" counts={overview.counts.creatureNpcs} available={overview.availability.creatureNpcs} emptyMessage="No Creature NPCs found.">
    ...
    <details className="group rounded-3xl border border-white/10 bg-black/35 p-6 shadow-2xl backdrop-blur-md sm:p-7">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-amber-300/60 [&::-webkit-details-marker]:hidden">
        <span>
          ...
          <span className="mt-1 block text-sm text-slate-400">{available ? `${counts.active} active / ${counts.archived} archived` : "Not available yet"}</span>
        </span>
        <span className="flex items-center gap-3">
          <span className="rounded-full border border-amber-300/25 bg-amber-300/10 px-3 py-1 text-sm text-amber-200">{available ? counts.total : "Pending"}</span>
      ...
      <div className="pt-5">
        {counts.total > 0
          ...
          : <p className="rounded-xl border border-dashed border-white/10 px-4 py-5 text-sm text-slate-400">{available ? emptyMessage : "This system is not available in the rebuild yet."}</p>}
      </div>
    </details>
  );
}
